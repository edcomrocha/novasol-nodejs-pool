(function ($) {
    ...
    $.updateHome = function (updateKey) {
        ...
        if (stats) {
            ...
            $.updateText(`poolMiners${updateKey}`, `Pool: ${stats.pool.miners} — Solo: ${stats.pool.minersSolo}`);
            $.updateText(`blocksTotal${updateKey}`, `Pool: ${stats.pool.totalBlocks} — Solo: ${stats.pool.totalBlocksSolo}`);
    ...
    $.initTemplate = function (parentStats, siblingStats) {
        ...
        Object.keys(siblingStats)
            .forEach(key => {
                ...
                $.updateText(`poolMiners${key}`, `Pool: ${siblingStats[key].pool.miners} — Solo: ${siblingStats[key].pool.minersSolo}`);
                $.updateText(`blocksTotal${key}`, `Pool: ${siblingStats[key].pool.totalBlocks} — Solo: ${siblingStats[key].pool.totalBlocksSolo}`);
        ...
        $.updateText('hashPower', hashPower.toFixed(2) + '%');
        $.updateText(`poolMiners${coin}`, `Pool: ${parentStats.pool.miners} — Solo: ${parentStats.pool.minersSolo}`);
        $.updateText(`blocksTotal${coin}`, `Pool: ${parentStats.pool.totalBlocks} — Solo: ${parentStats.pool.totalBlocksSolo}`);
        ...
        if (data.hasOwnProperty("charts")) {
            ...
            for (let graphType in graphData) {
                if (graphData[graphType].data[0].values.length > 1) {
                    ...
                    settings.tooltipFormat = graphData[graphType].data[1] ? '<span style="color: rgb(51,94,234);">POOL: {{y}}</span> &ndash; {{offset:names}}' : '<span>{{y}}</span> &ndash; {{offset:names}}';
