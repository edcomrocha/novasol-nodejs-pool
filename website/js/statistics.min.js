(function ($) {
  ...
  let graphSettings = {
    ...
    width: '100%',
    height: '140',
    lineColor: 'hsl(210 25% 28% / 1)',
    fillColor: 'hsl(210 25% 30% / 1)',
    spotColor: null,
    minSpotColor: 'hsl(210 25% 45% / 1)',
    maxSpotColor: 'hsl(210 25% 55% / 1)',
  ...
  $.statsFetchAddressStats = function (longPoll, stats, api, xhrAddressPoll) {
    ...
    xhrAddressPoll[stats.config.coin] = $.ajax({
      ...
      success: function (data) {
        ...
        $.updateText(`yourHashrateHolderPercentage${stats.config.coin}`, `${(((data.stats.hashrate / parseInt((stats.network.difficulty / stats.config.coinDifficultyTarget).toFixed(0))) * 100).toFixed(2) == "Infinity" ? "0.00" : ((data.stats.hashrate / parseInt((stats.network.difficulty / stats.config.coinDifficultyTarget).toFixed(0))) * 100).toFixed(2))}%`);
