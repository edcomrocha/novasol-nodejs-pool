(function ($) {
    ...
    $.marketCalcEstimateProfit = function(marketPrices) {
        ...
        let networkHash = (lastStats.network.difficulty / lastStats.config.coinDifficultyTarget);
        let coin = lastStats.config.coin;
        try {
            ...
            let profit = ((86400 / lastStats.config.coinDifficultyTarget) * lastStats.lastblock.reward) * (hashRate / networkHash >= 1 ? 1 : hashRate / networkHash);
            //let profit = (hashRate * 86400 / lastStats.network.difficulty) * lastStats.lastblock.reward;
            if (profit) {
                $.updateText(`calcHashAmount${coin}1`, parseFloat((profit / (10 ** lastStats.config.coinDecimalPlaces))).toLocaleString("en-US", { minimumFractionDigits: 5, maximumFractionDigits: 5 }, ".", ",") + " " + lastStats.config.symbol);
                $.updateText(`calcHashAmount${coin}2`, $.marketGetCurrencyPriceText(lastStats, profit, marketPrices));
                //return;
            } else {
                $.updateText(`calcHashAmount${coin}1`, '0.00000 ' + lastStats.config.symbol);
