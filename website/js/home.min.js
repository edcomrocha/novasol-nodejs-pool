(function ($) {
  ...
  let graphSettings = {
      ...
      width: '100%',
      height: '190',
      lineColor: 'hsl(210 25% 38% / 1)',
      fillColor: 'hsl(210 25% 38% / 0.25)',
      spotColor: null,
      minSpotColor: null,
      maxSpotColor: null,
      highlightLineColor: 'hsl(210 25% 40% / 1)',
      spotRadius: 0,
      chartRangeMin: 0,
  ...
  $.updateHome = function (updateKey) {
      ...
      if (stats) {
          ...
          $.updateText(`networkHashrate${updateKey}`, $.getReadableHashRateString(stats.network.difficulty / stats.config.coinDifficultyTarget) + '/s');
          ...
          $.updateText(`poolMiners${updateKey}`, `Pool: ${stats.pool.miners} (${stats.pool.workers}) — Solo: ${stats.pool.minersSolo} (${stats.pool.workersSolo})`);
          ...
          $.updateText('poolHashrateSolo', $.getReadableHashRateString(lastStats.pool.hashrateSolo) + '/s');
  ...
  $.initTemplate = function (parentStats, siblingStats) {
      ...
      if ($(`#networkStats${coin}`).length === 0) {
          minerInfo.push({
              ...
              workers: parentStats.pool.workers.toString(),
              workersSolo: parentStats.pool.workersSolo.toString()
          });
      ...
      $.updateText(`networkHashrate${coin}`, $.getReadableHashRateString(parentStats.network.difficulty / parentStats.config.coinDifficultyTarget) + '/s');
      ...
      Object.keys(siblingStats)
          .forEach(key => {
              ...
              minerInfo.push({
                  ...
                  workers: siblingStats[key].pool.workers.toString(),
                  workersSolo: siblingStats[key].pool.workersSolo.toString()
              });
              ...
              $.updateText(`networkHashrate${key}`, $.getReadableHashRateString(siblingStats[key].network.difficulty / siblingStats[key].config.coinDifficultyTarget) + '/s');
              ...
              $.updateText(`poolMiners${key}`, `Pool: ${siblingStats[key].pool.miners} (${siblingStats[key].pool.workers}) — Solo: ${siblingStats[key].pool.minersSolo} (${siblingStats[key].pool.workersSolo})`);
      ...
      $.updateText('poolHashrate', `${$.getReadableHashRateString(parentStats.pool.hashrate)}/s`);
      $.updateText('poolHashrateSolo', `${$.getReadableHashRateString(parentStats.pool.hashrateSolo)}/s`);
      ...
      $.updateText('hashPower', hashPower.toFixed(2) + '%');
      $.updateText(`poolMiners${coin}`, `Pool: ${parentStats.pool.miners} (${parentStats.pool.workers}) — Solo: ${parentStats.pool.minersSolo} (${parentStats.pool.workersSolo})`);
      ...
      if (data.hasOwnProperty("charts")) {
          let graphData = {
              hashrate: {
                  ...
                  options: {
                      lineColor: 'hsl(110 24% 38% / 1)',
                      fillColor: 'hsl(110 24% 38% / 0.25)'
                  }
              },
              diff: {
          ...
          for (let graphType in graphData) {
              if (graphData[graphType].data[0].values.length > 1) {
                  ...
                  settings.tooltipFormat = graphData[graphType].data[1] ? '<span style="color: #90A0AF">POOL:</span> {{y}} &ndash; {{offset:names}}' : '<span>{{y}}</span> &ndash; {{offset:names}}';
                  ...
                  if (graphData[graphType].data[1]) {
                      ...
                      settings.tooltipFormat = '<span style="color: #90A0AF;">SOLO:</span> {{y}} &ndash; {{offset:names}}';
