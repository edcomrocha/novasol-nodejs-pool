 ...
 * https://github.com/NovaSolNetwork/novasol-nodejs-pool
 *
 * Pool API
 **/
...
function getNetworkData(callback, rpcMode) {
    ...
    if (rpcMode === 'getinfo') {
        apiInterfaces.rpcDaemon('getinfo', {}, function (error, reply) {
            log('info', logSystem, 'getinfo', [reply]);
            if (error) {
...
function broadcastWorkerStats(address, destinations) {
    ...
    log('info', logSystem, 'Error polling getblocktemplate %j', [redisCommands]);
    redisClient.multi(redisCommands)
        .exec(function (error, replies) {
...
function initMonitoring() {
    ...
    for (let module in config.monitoring) {
        ...
        //     break
        // }
        if ((daemonType === "bytecoin" || daemonType === "cirquity") && module === "wallet" && settings.rpcMethod === "getbalance") {
