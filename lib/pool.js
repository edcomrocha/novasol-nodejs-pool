...
if (config.daemonType === 'bytecoin' || config.daemonType === "cirquity") {
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate, pool) {
    ...
    log('info', logSystem, 'Accepted %s share at difficulty %d/%d from %s@%s', [shareType, job.difficulty, shareDiff, login, miner.ip.split(':')[miner.ip.split(':').length - 1]]);
}
